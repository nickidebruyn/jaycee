(function () {
  ...
  function QuoteCreateController ($location, $routeParams, validationUtils) {
    ...
    function save() {
      ...
      doc.addImage(imgData, 'JPEG', 10, 20, 52, 13);
    ...
    function addDocRow(doc, item, count) {
      ...
      doc.setTextColor(0);
      doc.setFontSize(8);
      doc.text(10, 40 + (count * spacing), item.name);
      doc.text(50, 40 + (count * spacing), item.size);
      doc.text(80, 40 + (count * spacing), item.qty + "");
      doc.text(90, 40 + (count * spacing), item.description);
      doc.text(150, 40 + (count * spacing), item.unitPrice + "");
      doc.text(180, 40 + (count * spacing), getItemTotalPrice(item) + "");
